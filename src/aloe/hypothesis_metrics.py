...
class FnMetric(HypothesisMetric):
    ...
    class _State:
        def __init__(self, hm, clause, k=0):
            """
            ...
            self.h = 0 # TODO
            self.p = len([e for e in hm.E['pos'] if len(hm.solver.solve(e.head, self.B, verbose=0))>0])
            self.n = len([e for e in hm.E['neg'] if len(hm.solver.solve(e.head, self.B, verbose=0))>0])
            self.g = self.p - self.c - self.h
            self.f = self.g - self.n
            
            self.metrics = (self.c,self.h,self.p,self.n,self.g,self.f)
            
        def copy(self): return self.State(self.clause.copy(), self.k)
        def __str__(self): 
            s = '[%3d,%3d,%3d,%3d,%3d,%3d]' % self.metrics
            return '%s Clause(k=%d):%s' % (s, self.k, str(self.clause))
